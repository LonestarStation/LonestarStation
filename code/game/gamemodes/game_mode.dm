...
/datum/game_mode/Topic(href, href_list[])
	...
	if(href_list["toggle"])
		...
		if(href_list["debug_antag"] == "self")
			...
			return
		var/datum/antagonist/antag = SSantags.antag_datums[href_list["debug_antag"]]
		if(antag)
		...
		if(antag_tags && (href_list["remove_antag_type"] in antag_tags))
			...
			return
		var/datum/antagonist/antag = SSantags.antag_datums[href_list["remove_antag_type"]]
		...
		var/choice = input("Which type do you wish to add?") as null|anything in SSantags.antag_datums
		if(!choice)
			return
		var/datum/antagonist/antag = SSantags.antag_datums[choice]
		if(antag)
...
/datum/game_mode/proc/can_start(var/do_not_spawn)
	...
	if(antag_tags && antag_tags.len)
		for(var/antag_tag in antag_tags)
			var/datum/antagonist/antag = SSantags.antag_datums[antag_tag]
			if(!antag)
				continue
...
/datum/game_mode/proc/get_players_for_role(var/role, var/antag_id, var/ghosts_only)
	...
	var/datum/antagonist/antag_template = SSantags.antag_datums[antag_id]
	if(!antag_template)
		return candidates
