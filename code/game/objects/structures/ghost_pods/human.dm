...
//Lonestar Ed.//
//for Lonestar raccoons//

/obj/structure/ghost_pod/ghost_activated/trash_pile
	name = "trash pile"
	desc = "This large pile of trash seems to rustle a little when you're not looking... is something inside?"
	description_info = "This trash could contain a hungry little critter, which may wake at any time.\
	The pile can be searched, but there might not be anything here."
	icon = 'icons/obj/structures.dmi'
	icon_state = "rac-trash"
	icon_state_opened = "rac-trash"
	density = FALSE
	ghost_query_type = /datum/ghost_query/raccoon
	anchored = TRUE

	var/occupant_type = "raccoon"
...
/obj/structure/ghost_pod/ghost_activated/trash_pile/Initialize()
	. = ..()

	handle_clothing_setup()

/obj/structure/ghost_pod/ghost_activated/trash_pile/proc/handle_clothing_setup()
	clothing_possibilities = list()

	clothing_possibilities |= /obj/item/storage/backpack/raccpack

/obj/structure/ghost_pod/ghost_activated/trash_pile/create_occupant(var/mob/M)
	..()
	var/turf/T = get_turf(src)
	var/mob/living/carbon/human/raccoon/H = new(src)

	to_chat(M, "<span class='notice'>You are a [occupant_type]!</span>")
	to_chat(M, "<span class='warning'>You are a fuzzy little rascal. You'll be hungry soon.</span>")
	H.ckey = M.ckey
	visible_message("<span class='warning'>As \the [src] rustles, the trashbags in \the [src] part and something crawls out...</span>")
	log_and_message_admins("successfully opened \a [src] and became a [occupant_type].", H)

	var/list/backpack_options

	if(clothing_possibilities && clothing_possibilities.len)
		for(var/path in clothing_possibilities)
			if(ispath(path, /obj/item/storage/backpack))
				if(!backpack_options)
					backpack_options = list()
				backpack_options |= path

	if(backpack_options && backpack_options.len)
		var/newpath = pick(backpack_options)
		var/obj/item/clothing/C = new newpath(H)
		H.equip_to_appropriate_slot(C)


	var/newname = sanitize(input(H, "You don't really have a name, but you think the locals might call you [H.real_name]. Would you like to change your name?", "Name change") as null|text, MAX_NAME_LEN)
	if (newname)
		H.real_name = newname
