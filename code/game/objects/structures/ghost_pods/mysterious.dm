...
/*
//Lonestar Ed. //saving this for now
/obj/structure/ghost_pod/automatic/ls_raccoon
	name = "pile of trash"
	desc = "Looks like a pile of trash."
	description_info = "It seems like it happens to all be food related trash."
	icon_state = "raccoon_stash"
	icon_state_opened = "raccoon_stash"
	density = FALSE
	anchored = TRUE
	ghost_query_type = /datum/ghost_query/ls_raccoon
	needscharger = FALSE

/obj/structure/ghost_pod/automatic/ls_raccoon/create_occupant(var/mob/M)
	density = FALSE
	var/mob/living/carbon/human/raccoon/R = new(get_turf(src))
	if(M.mind)
		M.mind.transfer_to(R)
	// Put this text here before ckey change so that their laws are shown below it, since borg login() shows it.
	to_chat(M, "<span class='notice'>You are a <b>Lonestar raccoon</b>, your people have lived on this asteroid for generations.</span>")
	to_chat(M, "<span class='notice'><b>Be be careful. The humans cannot be trusted easily.</b></span>")
	R.ckey = M.ckey
	visible_message("<span class='warning'>A large raccoon crawls out of \the [src].</span>")
//	R.Namepick()
	..()
	*/