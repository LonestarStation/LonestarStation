...
/datum/event/rogue_drone/announce()
	...
	switch(rng)
		if(1)
			msg = "A SolGov combat drone wing operating around the main belt has failed to return from an anti-piracy sweep.  If any are sighted, \
			approach with caution."
		if(2)
			msg = "Contact has been lost with a SolGov combat drone wing in the main belt.  If any are sighted in the area, approach with \
			caution."
		if(3)
			msg = "Unidentified hackers have targeted a SolGov combat drone wing deployed near the main belt. If any are sighted in the area, approach \
			with caution."
		if(4)
			msg = "A passing derelict ship's drone defense systems have just activated. If any are sighted in the area, use caution."
		if(5)
			msg = "We're detecting a swarm of small objects approaching.  Most likely a bunch of drones.  Please exercise caution if you see any."
